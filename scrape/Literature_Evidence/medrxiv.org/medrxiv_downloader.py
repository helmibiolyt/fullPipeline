#!/usr/bin/env python3
"""
medRxiv Preprint Metadata Downloader
Author: Antigravity AI Coding Assistant
Description: Extracts preprint metadata from the Cold Spring Harbor Laboratory (CSHL) medRxiv API,
...
# api.medrxiv.org publishes no hard limit; keep it gentle for a free public API.
MAX_REQUESTS_PER_SECOND = 5.0

logger = logging.getLogger("medrxiv_downloader")
...
def fetch_api_page(server: str, start_date: str, end_date: str, cursor: int, retries: int = 5, delay: float = 1.0) -> dict:
    """
    Downloads a single page of metadata from CSHL medRxiv API with retries and exponential backoff.
    ...
    Format: https://api.medrxiv.org/details/[server]/[interval]/[cursor]/json
    """
    interval = f"{start_date}/{end_date}"
    url = f"https://api.medrxiv.org/details/{server}/{interval}/{cursor}/json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Scrape preprint metadata from Cold Spring Harbor Laboratory (CSHL) medRxiv API."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(BASE_DIR / "medrxiv"),
    ...
    log_file = output_path / "medrxiv_downloader.log"
